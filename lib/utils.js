 ...
 */
export async function get_node_executable(options) {
  ...
  const node_executable_filename = `node-${platform}-${arch}-v${nodeVersion}-with-intl-${withIntl}${platform === 'windows' ? '.exe' : ''}`
  console.log({node_executable_filename})
